...
df2["change"] = df2.clast.astype("float") / df2["count"].astype("float")
...
for i in df3.index:
    ...
    if len(x)<100:
        continue
    r = (x.close[len(x)-16] -  x.close[len(x)-82]) / x.close[len(x)-82]
    ndf["price_change1"][i] =r
...
print(df3[:20])
df3.to_csv(r"C:\Users\qeaw\Desktop\rrr\2017-10-29\result5.csv")