...
datadf = pd.read_table("SH#600526.txt", skiprows=2, skipfooter=1, header=None, engine='python',
                               parse_dates=False)
# print(datadf)
...
while True:
    now = datetime.now()
    r = requests.get('http://hq.sinajs.cn/list=sh600526')
    ...
    if now.hour >= 15:
        ...
        with open('dairy_record_new.txt', 'w') as f:
            f.write(str(result))
    break
