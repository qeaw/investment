...
datadf = pd.read_table("SZ#300008.txt", skiprows=2, skipfooter=1, header=None, engine='python',
                               parse_dates=False)
# print(datadf)
...
while True:
    now = datetime.now()
    r = requests.get('http://hq.sinajs.cn/list=sz300008')
    ...
    if now.hour == 10 and now.minute == 0:
        predq = q * 8
        if predq > maxmean:
            send_email(_user, _pwd, _to, "volume remaind", f"sell {gpdm}: current volume {q}")
            show(f"sell for predicted quantity is high at {predq}")
            result.append([q,p,now])
        if predq < minmean:
            send_email(_user, _pwd, _to, "volume remaind", f"buy {gpdm}: current volume {q}")
    ...
    if now.hour >= 15:
        ...
        with open('thfw'+str(datetime.now().day) + ".txt", 'w') as f:
            f.write(str(result))
        break
