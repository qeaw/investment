...
def print_daxie(number,daima,n,p,rate):
    num = int(number)
    num_str = str(num)
    leng = len(num_str)
    wei = ["元", "十", "百", "千", "万", "十万", "百万", "千万", "亿", "十亿", "百亿"]
    result = num_str[0] + wei[len(num_str) - 1]
    print(daima, n, result,p,rate)
...
def jiance(gpdm):
    ...
    minmean = datadf["成交额"][len(datadf) - 45:].min()
    maxmean = sum(sorted(datadf["成交额"][len(datadf) - 45:])[42:]) / 3
    ...
    while True:
        ...
        print(datetime.now())
        # print(gpdm)
        print_daxie(q, gpdm, name, p, rate)
        # send_email(_user,_pwd,_to,"volume remaind", f"current volume{q}")
        if now.hour == 10 and now.minute == 30:
            predq = q * 4
            if predq > maxmean:
                send_email(_user, _pwd, _to, "成交量提醒",
                           f"卖出 {name}{wenjianming}: 现成交量 {daxie(q)}, 现价{p}, 涨幅{rate}, 量比{round(q / minmean,2)}")
                ...
                itchat.search_chatrooms(name="triple_group")[0].send(
                    f"卖出 {name}{huifu(wenjianming)}: 现成交量 {daxie(q)}, 现价{p}, 涨幅{rate}, 量比{round(q / minmean,2)}")
            ...
            if predq < minmean and qnow!= 0 and p < lj* 1.03 and p > lj * 0.97 and dn > 0:
                send_email(_user, _pwd, _to, "成交量提醒",
                           f"买入 {name}{huifu(wenjianming)}: 现成交量 {daxie(q)}，现价{p}，涨幅{rate},量比{round(q / maxmean,2)}")
        ...
        if now.hour == 11 and now.minute == 25:
            ...
            if predq < minmean and qnow!= 0 and p < lj* 1.03 and p > lj * 0.97 and dn > 0:
                send_email(_user, _pwd, _to, "成交量提醒",
                           f"买入 {name}{wenjianming}: 现成交量 {daxie(q)}，现价{p}，涨幅{rate},量比{round(q / maxmean,2)}")
                # itchat.search_friends(name="禅语")[0].send(f"买入 {name}{gpdm}: 现成交量 {daxie(q)}，现价{p}，涨幅{rate},量比{round(q / maxmean,2)}")
                itchat.search_chatrooms(name="triple_group")[0].send(
                    f"买入 {name}{huifu(wenjianming)}: 现成交量 {daxie(q)}, 现价{p}, 涨幅{rate}, 量比{round(q / maxmean,2)}")
                show(f"buy for predicted quantity is low at {predq}")
        ...
        if q > maxmean and n < 1:
            send_email(_user, _pwd, _to, "成交量提醒",
                       f"现在卖出 {name}{wenjianming}: 现成交量 {daxie(q)}, 现价{p}, 涨幅{rate}, 量比{round(q / minmean,2)}")
            ...
            itchat.search_chatrooms(name="triple_group")[0].send(
                f"现在卖出 {name}{huifu(wenjianming)}: 现成交量 {daxie(q)}, 现价{p}, 涨幅{rate}, 量比{round(q / minmean,2)}(之后不再提醒)")
            result.append([q, p, now, "sell"])
            n += 1
            # time.sleep(1800)
        # if q < minmean:
        ...
        if now.hour >= 15:
            ...
            with open(gpdm + "d" + str(datetime.now().day) + ".txt", 'w') as f:
                f.write(str(result))
            break
